...
const { updateSheet, getSheetValues, getNamedRanges, addNewSheet } = require('./sheetsCrud')
...
exports.runAudit = async (auditStartDate, auditEndDate) => {
    ...
    const auditSheetName = `audit_${startDate}_${endDate}`

    await addNewSheet(`audit_${startDate}_${endDate}`)

    const auditUpdates = transformAuditResultsToUpdates(dfAuditFinal.values, `audit_${startDate}_${endDate}`)

    await updateSheet(auditUpdates)
}
