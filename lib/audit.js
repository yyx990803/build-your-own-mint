...
exports.runAudit = async (auditStartDate, auditEndDate) => {
    ...
    const auditSheetName = `audit_(${startDate}-${endDate})_${moment().toISOString()}`

    await addNewSheet(auditSheetName)

    const auditUpdates = transformAuditResultsToUpdates(dfAuditFinal.values, auditSheetName)
