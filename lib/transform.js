...
exports.transformTransactionsToUpdates = function(transactions) {
  ...
  const updates = transactions.map(function(transaction, i) {
    return {
      range: `${process.env.SHEETS_TRANSACTIONS_SHEET_NAME}!A${i + 2}:F${i + 2}`,
      values: [Object.values(transaction)]
    }
  });

  updates.push({
    range: `${process.env.SHEETS_TRANSACTIONS_SHEET_NAME}!A1:F1`,
    values: [['Account', 'Name', 'Date', 'Timestamp', 'Amount', 'Category']]
  })
